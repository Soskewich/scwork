...
import style from './Rids.module.css';
import styleSearch from '../Helpers/Search/Search.module.css';
import {useDispatch, useSelector} from "react-redux";
import {fetchRids, fetchRidsSearch} from "../../store/slices/RidsSlice";
...
const Rids = () => {
    const {rids, pageSize, count, isFetching, data} = useSelector(state => state.rids);
    ...
    const handlePageClick = (e) => {
        dispatch(fetchRidsSearch({search, author_id: data.authorName, from_date: data.beforeTime,
            to_date: data.afterTime, page: e.selected, pageSize}));
    }

    React.useEffect(() => {
        if (debouncedSearch[0] !== '') {
            dispatch(fetchRidsSearch({search, author_id: data.authorName,
                from_date: data.beforeTime, to_date: data.afterTime, page: 0, pageSize}))
        } else {
            dispatch(fetchRids({page: 0, pageSize}))
        }
    ...
    return (<div className={style.container}>
            ...
            <div className={style.blockCount}>
                <div>Количество найденных <br></br>РИД: <span className={style.count}>{count}</span></div>
            </div>
            {/*<NioktrFilterSize min={20} mid={40} max={80}/>*/}
            {isFetching === true ? <Preloader/> :
                <div className={style.block}>
                    {rids.map((item, index) => <div key={index}>
                        <div className={style.blocks}>
                            <NavLink to={"/rid/" + item.id}>
                                <div>{item.title}</div>
                            </NavLink>
                            ...
                            <div className={style.keyword_rids}>
                                Ключевые слова:
                                <div>
                                    {item.keyword_rids.map((wordItem, index) =>
                            ...
                            <div>
                                {item.rid_date === null ||
                                item.rid_date === undefined ? '' :
                                    item.rid_date.slice(0, 4)}
                            </div>
                        </div>
